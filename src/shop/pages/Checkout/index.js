  ...
  useEffect(() => {
    ...
    const fetchDataAddress = async () => {
      try {
        ...
        setAddressList([...addressData, { address: "Other Address", _id: "-1" }]);
      } catch (error) {
        console.log(error);
      }
    };
        ...
        case 2:
          {
            try {
              ...
              const { payUrl } = await orderApi.getPayUrlPayPal({ amount: cartData?.total, paymentId })
              await orderApi.create(body)
              await userApi.updateCart(currentUser?.userId, { cart: [] })
              setLoading(false)
              window.location.href = payUrl
    ...
    <div className="main">
      ...
      <Container>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active linkAs={NavLink} linkProps={{ to: "/thanh-toan" }}>Pay</Breadcrumb.Item>
        </Breadcrumb>
              ...
              <div className={styles.payment_form}>
                ...
                {shippingAddress && shippingAddress?.districtId ? (
                  <>
                    ...
                    {leadTime && <p>Estimated delivery time: <Badge bg="danger">{moment.unix(leadTime).format("DD-MM-yyy")}</Badge></p>}
                  </>
                ) : null}
                <br></br>
