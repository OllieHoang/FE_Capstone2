...
function BookList() {
  ...
  const handleCallApiDelete = async (e) => {
    try {
      ...
      if (orders.length > 0) {
        toast.error('The product has been purchased, cannot be deleted!', { autoClose: 2000 })
        return
      }
  ...
  return (
    <Row>
      <Modal size="lg" show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Body>Are you sure to delete the tag<b>{bookDelete && bookDelete?.name}</b> this?</Modal.Body>
        <Modal.Footer>
      ...
      <Col xl={12}>
        <div className="admin-content-wrapper">
          <div className="admin-content-header">List of products</div>
          <div className="admin-content-action">
            <div className="d-flex">
              <input className="form-control search" placeholder="Seacher" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} />
