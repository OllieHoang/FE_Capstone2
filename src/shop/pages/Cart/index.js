  ...
  const handleApplyVoucher = async () => {
    try {
      ...
      if (cartData.subTotal < minimum) {
        toast.info(
          `Minimum order value required${format.formatPrice(minimum)} to apply!`,
          { autoClose: 2000 })
        return
      }
      const now = new Date()
    ...
    <div className="main">
      <Container>
        <Breadcrumb>
          ...
          <Breadcrumb.Item active linkAs={NavLink} linkProps={{ to: "/gio-hang" }}>Cart</Breadcrumb.Item>
        </Breadcrumb>
              ...
              <div className={styles.cartRight}>
                <div className={styles.voucher}>
                  <div className={styles.voucherGroup}>
                    <input type="text" className="form-control" placeholder="Enter vocher code" value={voucherInput} onChange={(e) => setVoucherInput(e.target.value)} />
          ...
          <Row>
            <Col xl={12}>
              <div className={styles.empty}>
                ...
                <Link to="/" className={`bookstore-btn ${styles.backHome}`}>Continue shopping</Link>
              </div>
            </Col>
          </Row>}
      </Container>
